...
export default function App() {
  ...
  async function handleLikeRepository(id) {
    const response = await (await api.post(`repositories/${id}/like`)).data;
    const likes = response.likes;

    const reposiUdpt = repositories.map((repository) => {
      if (repository.id == id) return { ... repository, likes };
      else return repository;
    });

    setRepositories(reposiUdpt);
  }
